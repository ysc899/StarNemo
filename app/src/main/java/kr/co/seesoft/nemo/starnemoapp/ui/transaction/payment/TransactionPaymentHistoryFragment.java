...
public class TransactionPaymentHistoryFragment extends Fragment {
    ...
    private RadioButton raReceiptPrint, raReceiptNotPrint;
    ...
    private void initUI(View view) {
        ...
        rgReceipt = (RadioGroup) view.findViewById(R.id.rgReceipt);
...
//                }
                ...
                sbContents.append("--------------------------------\n");
                sbContents.append(String.format("물 품 가 액%,28d원\n", ((lTotalAmount - lSurtax - lTip)) * lMinus));
                if (lSurtax > 0)
                {
                    sbContents.append(String.format("부   가  세%,28d원\n", lSurtax * lMinus));
                }
                if (lTip > 0)
                {
                    sbContents.append(String.format("봉   사  료%,28d원\n", lTip * lMinus));
                }
                sbContents.append(String.format("합 계 금 액%,28d원\n", lTotalAmount * lMinus));
                sbContents.append("--------------------------------\n");
...
//                            Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.sign, options);
    ...
    protected void selectTask(NemoSalesApprovalListRO t){
        ...
        if( "approval".equals(t.getTradType()) && "N".equals(t.getCanceled()))
        {
            btnPaymentCancel.setVisibility(View.VISIBLE);
            tvReceipt.setVisibility(View.VISIBLE);
            rgReceipt.setVisibility(View.VISIBLE);
        }
        else
        {
            btnPaymentCancel.setVisibility(View.GONE);
            tvReceipt.setVisibility(View.GONE);
            rgReceipt.setVisibility(View.GONE);
        }
